...
class DecodeLogs {
  ...
  async perform(){
    ...
    try{
        ...
        await new TransactionLogModel().updateById(
            ...
            {
                decode_status: transactionLogsConstants.successDecodeStatus, 
                decoded_events: JSON.stringify(resp)}
            );            
    } catch(err){
        console.error(err);
        await new TransactionLogModel().updateById(
            oThis.transactionLog.id, 
            {decode_status: transactionLogsConstants.errorDecodeStatus}
            );  
  ...
  async _decodeLog(log){
    ...
    if (!contractAbiRow || !contractAbiRow.id) {
        ...
        console.warn(msg);
        return {decode_status: transactionLogsConstants.abiNotFoundDecodeStatus, message: msg};
    }
    ...
    return {decode_status: transactionLogsConstants.successDecodeStatus, decoded_log: formattedLog};
  }
