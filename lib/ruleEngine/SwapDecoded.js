...
class FormatSwap {
  ...
  getSwapSummary(swapEvents, txDetail) {
    ...
    return oThis.checkIfEqual( txDetail.highlightedEventTexts, swapSummarryArray, txDetail.transactionHash, swapEvents.length);
  }

  checkIfEqual(highlightedEventTexts, formattedTextArr, txHash, swapEventsLength) {
    const oThis = this;
  ...
  getSwapDetails(swapEvents) {
    ...
    for (let event of swapEvents) {
      ...
      // console.log("params map: ", paramsNameToValueMap);
  ...
  async fetchTransactionDetailObj(limit, offset) {
    ...
    let transactionDetails = await fetchTransactionDetailObj
      ...
      // .where(['transaction_hash = "0x6b8c5f45fd0d7705826bbb2e3171ff7a9bedfd339371bf17e160eb2c72cf338f"'])
