...
class FormatSwap {
  ...
  getSwapSummary(swapEvents, txDetail) {
    ...
    console.log("swapSummarryFrom ETherscan: ", txDetail.highlightedEventTexts);
    console.log("swapSummarryArray: ", swapSummarryArray);

    return oThis.checkIfEqual( txDetail.highlightedEventTexts, swapSummarryArray);
  }
  ...
  checkIfEqual(highlightedEventTexts, formattedTextArr, kind) {
    ...
    console.log("Swap summary not matched-->")
  ...
  getSwapDetails(swapEvents) {
    ...
    for (let event of swapEvents) {
      ...
      if (methodId == "d78ad95f") {
        swapDetail.incomingAmount = oThis.addNumbers( paramsNameToValueMap["amount0In"], paramsNameToValueMap["amount1In"]);
        swapDetail.outgoingAmount = oThis.addNumbers( paramsNameToValueMap["amount0Out"], paramsNameToValueMap["amount1Out"]);
      } else {
  ...
  formatNumber(number, decimal = 18) {
  ...
  async fetchTransactionDetailObj(limit, offset) {
    ...
    let transactionDetails = await fetchTransactionDetailObj
      ...
      // .where(['transaction_hash = "0x0121e815e30e9c7441564cff71e8c8b0febd6bef91dc1f9411cac8fd45525d41"'])
      .limit(limit)
      .offset(offset)
      .fire();
