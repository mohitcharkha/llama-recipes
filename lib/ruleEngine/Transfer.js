...
class FormatApproval {
  ...
  perform(txDetail) {
    ...
    if (txDetail.logsData["items"].length == 1 && data.token_transfers && data.token_transfers.length == 1) {
      const itemData = logsData.items[0];     
        const decodedEventName = oThis.getMethodName(itemData);
        if (data.method == "transfer" || decodedEventName == "Transfer") {
