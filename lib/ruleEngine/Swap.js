  ...
  perform(txDetail) {
    ...
    if (swapEvents.length == 0) {
      if (isEthercanTextSwap) {
        ...
        if (txDetail.tempLogsData["items"].length > 0) {
          // console.log('SwapInEtherscanNotInScript::', txDetail.transactionHash);
          return oThis.getAllCounts(false, 'SwapInEtherscanNotInScript');
        } else {
          return oThis.getAllCounts(false, 'ZeroEvents');
        }
      } 
    ...
    for (let swapEvent of swapEvents) {
      ...
      const swapSummarry2 = `Swap ${incomingAmount.toString()} ${incomingTokenAddress} For ${outgoingAmount.toString()} ${outgoingTokenAddress}`;
      ...
      if (!equal) {
        ...
        if (!oThis.checkIfHighlightedEventTextIsSwap(highlightedEventTexts) && oThis.checkIfGeneratedTextIsSwap(swapSummarry)) {
          ...
          return oThis.getAllCounts(false, 'SwapInScriptNotInEtherscan');
        } else { 
          // const decimalDifference = oThis.check4DigitsEqual(highlightedEventTexts, swapSummarry2);
          ...
          return oThis.getAllCounts(false, 'AmountsNotEqual');
        } 
