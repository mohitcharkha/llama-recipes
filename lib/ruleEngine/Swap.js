...
class FormatSwap {
  ...
  perform(txDetail) {
    ...
    // Known : There are records having 2 swap events but single swap lanaguage generated, so they are coming in script_classification_failed_for_given_action
    if (swapEvents.length > 1) {
      if (oThis.isEthercanTextSwap) {
        console.log("2 swap single language", txDetail.transactionHash);
      }
