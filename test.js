...
function getTopicString(topics) {
  let topicString = topics.filter(topic => topic != null).join(", ");
  return topicString;
}
...
for (i = 0; i < a.length; i++) {
  ...
  for (j = 0; j < input.event_logs.length; j++) {
    ...
    const event_log_string = `${
      input.event_logs[j].address.hash
    }(${sanitizeString(
      input.event_logs[j].address?.name ?? "NA"
    )}) emitted "${getTopicString(input.event_logs[j].topics)}" on position ${
      input.event_logs[j].index
    }`;
  ...
  // with data "${input.event_logs[j]?.data ?? "NA"}"
  ...
  string_input += `from: ${input.transactions.from.hash} to: ${input
  ...
  b[i].instruction =
    "You are an expert in Ethereum blockchain and can explain transaction in one line.";
  b[i].input = string_input;
  let output = JSON.parse(a[i].output);
  b[i].output = output.join(",");
}
fs.writeFileSync("alpaca_data_set1.json", JSON.stringify(b));
