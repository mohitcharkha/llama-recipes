...
async function main() {
    ...
    for (i = 0; i < trainingDataset.length; i++) {
        ...
        let string_input = "";
        const instruction = "You are an expert in Ethereum blockchain and can classify the transactions into their specific categories based on the pattern.Give blank output for cases where transactions might not fall into any category";
    ...
    fs.writeFileSync("Datafiles/training_dataset_alpaca_data_2800_supportedTypes.json", JSON.stringify(finalOutput));
    fs.writeFileSync("Datafiles/training_dataset_hashes_2800_supportedTypes.json", JSON.stringify(transcationHashArray));
}
main().then(() => {
    console.log("done");
})
